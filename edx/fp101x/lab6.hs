...
instance Foldable Rose where
  fold (x :> xs) = x `mappend` (foldr (mappend) mempty (fmap fold xs))
...
foldf1 f a bs = foldr (\b -> \g -> (\a -> g (f a b))) id bs a
foldf2 f a bs = foldr (\a b -> f b a) a bs
foldf3 f = flip $ foldr (\a b g -> b (f g a)) id
foldf4 = foldr . flip
