...
digitToInt c = fromIntegral (ord c - ord '0')
...
checkToDigits n = (eval (toDigits n) == n) && (all (\d -> d >= 0 && d < 10) (toDigits n))
...
checkToDigitsRev n = (evalRev (toDigitsRev n) == n) && (all (\d -> d >= 0 && d < 10) (toDigitsRev n))
...
checkSumDigits = sumDigits [3, 9, 4, 15, 8] == 30
...
isValid n = result `mod` 10 == 0
    where result = sumDigits (doubleSecond (toDigitsRev n))
